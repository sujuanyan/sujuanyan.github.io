...
where $c_0$ is a constant factor that only depends on $c$. And for any $\alpha \in (0,1)$, the bound is $O(G^2/\lambda T)$ which approaches the optimal guarantees up to constant factors. The table below summaries the convergence rate for the different methods and different assumptions discussed in the paper. 

|              | strongly convex    | non-smooth         | rate          |
| ------------ | ------------------ | ------------------ | ------------- |
| SGD-A        | &check; | &cross; | $O(1/T)$      |
|              | &check; | &check; | $O(log(T)/T)$ |
| SGD-$\alpha$ | &check; | &check; | $O(1/T)$      |
| EPOCH-GD     | &check; | &check; | $O(1/T)$      |
